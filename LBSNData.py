...
class LBSNData(Dataset):
    ...
    def processing(self, data_name, data_path, min_user_freq, map_level):
        ...
        for user, seq in user_seq.items():
            if len(seq) >= min_user_freq:
                ...
                for loc, quadkey, lat, lng, timestamp in sorted(seq, key=lambda e: e[-1]):
                    if loc in tmp_set:
                        seq_new.append(
                            (user_idx, loc, quadkey, timestamp, lat, lng, True))
                    else:
                        seq_new.append(
                            (user_idx, loc, quadkey, timestamp, lat, lng, False))
                        tmp_set.add(loc)
                        cnt += 1
        ...
        for u in range(len(user_seq_array)):
            ...
            for i in range(len(seq)):
                ...
                user_seq_array[u][i] = (
                    check_in[0], check_in[1], region_quadkey_bigram, check_in[3], check_in[4], check_in[5], check_in[6])
        ...
        all_time_ngram = []
        times_tamp2time_ngrams = {'<pad>': '<pad>'}
        for user in range(len(user_seq_array)):
            seq = user_seq_array[user]
            for i in range(len(seq)):
                timestamp = seq[i][3]
                dt = datetime.datetime.fromtimestamp(timestamp)
                # year = '{:02d}'.format(dt.year % 100)
                month = dt.month
                weekday = dt.weekday()
                day = dt.day
                hour = dt.hour
                minute = dt.minute // 15
                # second = dt.second
                # timecode = [month, day, weekday, hour, minute]
                time_ngram = [month,
                               day,weekday, hour, minute]
                if timestamp not in times_tamp2time_ngrams.keys():
                    times_tamp2time_ngrams[timestamp] = time_ngram
                    self.n_timestamp += 1
                all_time_ngram.append(time_ngram)
                user_seq_array[user][i] = [seq[i][0], seq[i][1], seq[i][2],
                                           seq[i][3], seq[i][4], seq[i][5], time_ngram, seq[i][6]]
        self.TIMECODE = Field(
            sequential=True,
            use_vocab=True,
            batch_first=True,
        ...
        self.TIMECODE.build_vocab(all_time_ngram)
        ...
        for user in tqdm(range(len(self))):
            ...
            for k in range(n_instance):
                if (i - k * max_len) > max_len * 1.1:
                    ...
                    train_src_seq = seq[i - (k + 1) *
                                        max_len - 1: i - k * max_len - 1]
                    ...
                    #                                   spatial_matrix)
                    # train_seq.append(
                    #     (train_src_seq, train_trg_seq, t_mat, s_mat))
                else:
                    train_trg_seq = seq[max(
                        i - (k + 1) * max_len, 0): i - k * max_len]
                    train_src_seq = seq[max(
                        i - (k + 1) * max_len - 1, 0): i - k * max_len - 1]
                    train_seq.append((train_src_seq, train_trg_seq))
    ...
    def spatial_temporal_matrix_building(self, path):
        ...
        for u in tqdm(range(len(self))):
            seq = self[u]
            temporal_matrix, spatial_matrix = build_st_matrix(
                self, seq, len(seq))
